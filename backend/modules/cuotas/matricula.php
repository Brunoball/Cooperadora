...
// (Opcional) CORS simple. Quitá si no lo necesitás.
...
function ensure_matricula_exists(PDO $pdo, int $montoDefault = 15000): void {
    $stmt = $pdo->prepare("
        INSERT INTO cooperadora.meses (id_mes, nombre, monto)
...
try {
    ...
    if ($method === 'GET') {
        ...
        $stmt = $pdo->prepare("SELECT monto FROM cooperadora.meses WHERE id_mes = 14 LIMIT 1");
        $stmt->execute();
    ...
    if ($method === 'POST') {
        ...
        if (!isset($data['monto'])) {
            ...
            echo json_encode(['exito' => false, 'mensaje' => 'Falta el parámetro "monto".']);
            exit;
        }
        ...
        if ($monto < 0) {
            http_response_code(400);
            echo json_encode(['exito' => false, 'mensaje' => 'El monto no puede ser negativo.']);
            exit;
        }
        ...
        echo json_encode(['exito' => true, 'monto' => $monto, 'mensaje' => 'Matrícula actualizada.']);
        exit;
    }

    // Método no permitido
    http_response_code(405);
    echo json_encode(['exito' => false, 'mensaje' => 'Método no permitido. Usá GET o POST.']);
} catch (Throwable $e) {
